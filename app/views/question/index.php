...
<script>
    ...
    $check.on("click", function() {
        ...
        var element = blanks_number.find(item => parseInt(item.id) === idQuestion);
        if (element) {
                ...
                if (code === 200) {
                    $(".blank").each(function() {
                        ...
                        if (answerDetail) {
                            ...
                            $(this).css("background-color", isCorrect ? "lightgreen" :
                                "lightcoral");
                            var existingEntry = answers_tmp.find(entry => entry.question_id ===
                                idQuestion && entry.question_blank_id === blankId);
                            if (existingEntry) {
                ...
                success: function(data) {
                    ...
                    if (code === 200 && question) {
                        ...
                        questionText = questionText.replace(/<p>\s+/g, function(match) {
                            return '<p>' + '&nbsp;'.repeat(match.length -
                                3); // 3 là độ dài của <p> và 1 khoảng trắng mặc định
                        });
