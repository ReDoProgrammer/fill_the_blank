<div class="card">
    ...
    <div class="card-body">
        <div class="containter" id="profile" data-id="<?php echo $profile['id']; ?>" data-teaching="<?php echo $profile['teaching_id'];?>" data-usercode="<?php echo $profile['user_code'];?>">
            <div class="row mb-3">
...
<script>
    $(document).ready(function () {
        $('#btnSubmit').click(function () {
            ...
            $.ajax({
                url: '<?php echo BASE_URL; ?>/teacherauth/update',
                type: 'POST',
                dataType: 'json',
                data: {
                    id, username, fullname, new_password, phone, email,
                    user_code:$('#profile').attr('data-usercode'),teaching_id:parseInt($('#profile').attr('data-teaching'))
                },
                success: function (response) {
                    ...
                    if (code === 200) {
                        Swal.fire({
                            ...
                            window.location.href = `<?php echo BASE_URL;?>/teacher/auth/login`;
                        });
                    } else {
                        Swal.fire({
                            title: `Lỗi: ${code}`,
