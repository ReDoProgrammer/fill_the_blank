...
<div class="container">
    ...
    <script>
        ...
        var questions = [];
        const $btnSearch = $('#btnSearch'), $btnSubmit = $('#btnSubmit'), $keyword = $('#txtKeyword'),
            $table = $('#tblData'), $pagination = $('.pagination'),
            $modal = $('#modal'), $modalTittle = $('#modalTitle'),
            $questionsList = $('#questions');
        const $subjects = $('#selSubject'), $slConfigs = $('#slConfigs'), $slQuestions = $('#slQuestions');
        ...
        const LoadConfigs = function () {
            ...
            $.ajax({
                ...
                success: function (response) {
                    const { code, configs } = response;

                    configs.forEach(cf => {
                        $slConfigs.append(`<option value="${cf.id}">${cf.title} - [${cf.number_of_questions} câu, ${cf.marks} điểm ]</option>`);
                    })
                },
                error: function (err) {
                    console.log(err);
        ...
        function LoadData() {
            ...
            $.ajax({
                ...
                success: function (response) {
                    const { exams, currentPage, totalPages } = response;
                    ...
                    exams.forEach(e => {
                        ...
                        <tr class="align-middle">
                            ...
                            <td class="text-center">
                                <a href="javascript:void(0)" onClick="UpdateExam(${e.id})"><i class="fa fa-edit text-warning"></i></a>
                                ...
                                <a href="javascript:void(0)" onClick="DeleteExam(${e.id})"><i class="fa fa-trash-o text-danger"></i></a>
                                <a href="javascript:void(0)" onClick="ViewExamDetail(${e.id})"><i class="fa fa-eye" aria-hidden="true"></i></a>
                                <a href="javascript:void(0)" onClick="ViewQuestions(${e.id})"><i class="fa fa-list text-white bg-success"></i></a>
                            </td>
                        <tr/>
                    `);
