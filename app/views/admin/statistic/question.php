...
<div class="container mt-5">
    ...
    <!-- statistic Table -->
    <table class="table table-bordered table-striped table-hover mt-3 mb-5">
        <thead>
            <tr>
                <th scope="col">#</th>
                ...
                <th scope="col">Số lần trả lời đúng</th>              
            </tr>
        </thead>
...
<script>
    ...
    function LoadData() {
        $.ajax({
            url: '<?php echo BASE_URL; ?>/admin/statistic/get_questions_statistic',
            type: 'GET',
            dataType: 'json',
            data: {
                    ...
                    result.forEach(q=>{
                        $table.append(`
                            <tr>
                                ...
                                <td class="text-info fw-bold align-middle">${q.subject_name}</td>
                                <td class="align-middle fw-bold">${q.lession_name}</td>                            
                                <td>${formatDisplay(q.question_text)}</td>                            
                                <td class="align-middle">${q.total_exams}</td>                            
                                <td class="align-middle">${q.correct_answers}</td>                            
                            </tr>
                        `);
                    })
